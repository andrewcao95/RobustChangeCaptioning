...
class CaptionDataset(Dataset):
  def __init__(self, data_folder, data_name, split, captions_per_image):
    self.split = split
    ...
    if self.split == 'TEST':
      with open(os.path.join(data_folder, self.split + '_CAPTYPES_' + data_name + '.json'), 'r') as f:
        self.captypes = json.load(f)
  ...
  def __getitem__(self, i):
    ...
    #img_diff = torch.FloatTensor(self.imgs2[i // self.cpi] - self.imgs1[i // self.cpi])
    ...
    if self.split is 'TRAIN':
      ...
      captype = torch.LongTensor([self.captypes[i]])

      return img1, img2, caption, caplen, all_captions, captype
